...
for value in "$keys" "$operations" "$batch_size" "$value_bytes" "$range_width"; do
    ...
    for workload in "${workloads[@]}"; do
        if ((warmups > 0)); then
            for ((warmup = 1; warmup <= warmups; warmup++)); do
                warmup_label="warmup-${engine}-${workload}-${warmup}"
        ...
        for ((repetition = 1; repetition <= repetitions; repetition++)); do
            label="${engine}-${workload}-r${repetition}"
            run_benchmark "$(binary_for_engine "$engine")" "$engine" "$workload" "$label"
        done
    done
done

python3 - "$output_dir" "$repo_root" "$engines_csv" "$workloads_csv" \
    "$repetitions" "$warmups" "$keys" "$operations" "$batch_size" \
...
keys, operations, batch_size = map(int, sys.argv[7:10])
...
expected = len(engines) * len(workloads) * repetitions
...
for engine in engines:
    for workload in workloads:
        records = [record for record in runs if record["engine"] == engine and record["workload"] == workload]
        if len(records) != repetitions:
            raise SystemExit(f"expected {repetitions} records for {engine}/{workload}, found {len(records)}")
...
manifest = {
    "format": "seerdb-common-kv-qualification-v1",
