//! WAL record wire format and prefix parsing.
//!
...
/// Parse all complete records from a buffer prefix.
pub(crate) fn parse_records(buf: &[u8]) -> Vec<WalRecord> {
    parse_records_with_status(buf).0
}

/// Parse a WAL prefix and classify an incomplete or corrupt suffix.
pub(crate) fn parse_records_with_status(buf: &[u8]) -> (Vec<WalRecord>, ParseStatus) {
