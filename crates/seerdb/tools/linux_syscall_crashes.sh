...
# Kill a real SeerDB process while a selected libc durability call is held.
# The interposer marks either before the real call or immediately after a
# successful real call, then waits for the parent to send SIGKILL.

usage() {
    cat >&2 <<'EOF'
usage: tools/linux_syscall_crashes.sh [--output-dir PATH]

The default output is /tmp/seerdb-linux-syscall-crashes-TIMESTAMP. Each
observed fsync, fdatasync, rename, and write call is selected once before and
after completion in fresh whole-image and segmented databases. The parent
waits for the interposer marker, sends SIGKILL, and requires the old or
complete-new root after two fresh reopens.
EOF
}

die() {
    echo "linux_syscall_crashes: $*" >&2
    exit 2
}

repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
output_dir=${SEERDB_SYSCALL_CRASHES_OUTPUT:-"/tmp/seerdb-linux-syscall-crashes-$(date -u +%Y%m%dT%H%M%SZ)"}
...
for command in awk cargo cc cp git kill python3 sleep uname; do
    command -v "$command" >/dev/null || die "missing required command: $command"
done
[[ ! -e "$output_dir" ]] || die "output directory already exists: $output_dir"

mkdir -p "$output_dir/baselines" "$output_dir/runs"
build_root=$(mktemp -d "${TMPDIR:-/tmp}/seerdb-linux-syscall-crashes-build.XXXXXX")
cleanup() {
    local exit_code=$?
    rm -rf -- "$build_root"
    exit "$exit_code"
        ...
        for mode in before after; do
            for ((fault_after = 1; fault_after <= call_count; fault_after++)); do
                ...
                if (( ready == 0 )); then
                    ...
                    die "child did not reach hold marker for $case_name"
                fi
                ...
                [[ $child_status -eq 137 ]] || die "expected SIGKILL status 137 for $case_name, got $child_status"
                ...
                if ! SEERDB_POWERLOSS_BLOB_STORAGE="$layout" \
                    ...
                    echo "linux_syscall_crashes: verifier failed for $case_name" >&2
...
if not cases:
    raise SystemExit("no syscall crash cases executed")
...
manifest = {
    "format": "seerdb-linux-syscall-crash-manifest-v1",
    "repo_head": command_output("git", "-C", str(repo_root), "rev-parse", "HEAD"),
    "host_os": platform.system(),
    "host_arch": platform.machine(),
    "kernel": command_output("uname", "-r"),
    "termination": "parent sends SIGKILL after the interposer creates a hold marker",
    "boundary_modes": {
        "before": "marker is created before the selected real libc call",
        "after": "selected real libc call succeeds before marker creation and SIGKILL",
    },
    ...
    "not_exercised": [
        ...
        "machine power loss",
        "filesystem crash-consistency races outside the held libc call",
        "Rust positional page writes that bypass the interposed libc pwrite symbol",
    ],
    "cases": cases,
    "case_count": len(cases),
    "all_sigkill": all(item["child_exit_status"] == 137 for item in cases),
...
echo "linux_syscall_crashes: PASS manifest=$output_dir/manifest.json"
